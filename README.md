...
| 公厕台账 | `/restrooms`、`/restrooms/:id` | 台账增删改查、区域与状态筛选、公厕详情（档案 + 历史巡查 + 历史问题 + 位置调整记录）、点位调整留痕、关联数据删除保护 |
...
| 点位调整 | `/restroom-adjustments` | 全局点位调整留痕查询（区域/关键字/日期）；调整入口在公厕详情页 |
...
| POST | `/restrooms/{id}/location-adjustments` | 调整点位（区域/地址/经纬度），记录原因、操作人与完整前后值 |
| GET | `/restrooms/{id}/location-adjustments` | 单座公厕的点位调整历史（分页） |
| GET | `/restrooms/location-adjustments` | 点位调整记录全局查询（公厕/区域/操作人/关键字/日期，分页） |
...
- **点位调整与时点归属**：区域/地址/经纬度只能通过「位置调整」变更（普通编辑不可改），每次调整记录原因、操作人与完整前后值。每条巡查、问题在创建时固化当时的区域/地址/经纬度快照；调整只更新公厕当前位置，**不回写历史记录**。因此历史记录仍按发生时位置统计、口径在调整前后连续，调整记录可在公厕详情「位置调整」页签及全局页面查询。老库启动时会自动补列并按公厕当前位置回填历史快照。
